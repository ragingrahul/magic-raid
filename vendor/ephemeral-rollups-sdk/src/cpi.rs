...
fn delegate_account_inner<'a, 'info>(
    ...
    let (_, delegate_account_bump) =
        Pubkey::find_program_address(pda_seeds, accounts.owner_program.key.as_modern());

    let (_, buffer_pda_bump) =
        Pubkey::find_program_address(buffer_seeds, accounts.owner_program.key.as_modern());
...
pub fn undelegate_account<'a, 'info>(
    ...
    let (_, account_bump) =
        Pubkey::find_program_address(account_seeds.as_ref(), owner_program.as_modern());
...
fn cpi_delegate_with_discriminator<'a, 'info>(
    ...
    args.serialize(&mut data)
        .map_err(|_| compat::latest::ProgramError::BorshIoError(String::new()).compat())?;
